    ...
    try {
        ...
        if (!data) {
            return res.status(404).json({
                message: "Não foi possível encontrar o profissional, pois o mesmo não existe.",
                hasError: true,
            });
        }
        return res.status(200).json({
...
export const deleteOneWorker = async (req: Request, res: Response) => {
    try {
        ...
        if (!data) {
            return res.status(404).json({
                message: "Não foi possível deletar o profissional, pois o mesmo não existe.",
                hasError: true,
            });
        }
        return res.status(200).json({
...
export const updateWorker = async (req: Request, res: Response) => {
    try {
        ...
        if (
            ...
            return res.status(404).json({ hasError: true, message: 'Profissional não encontrado, não pode ser atualizado.' })
        }
        ...
        return res.status(500).json({ hasError: true, message: 'Erro interno do servidor' })
    }
}
